...
import requests  # HTTP 통신을 위해 추가
import datasets
import flwr as fl
import torch
...
class CifarClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # 로컬 학습 후 서버로 파라미터 전송
        self.send_parameters_to_server(parameters_prime)
    ...
    def send_parameters_to_server(self, parameters):
        ...
        data = {
            "client_id": client_id,
            "parameters": [p.tolist() for p in parameters]  # 파라미터를 JSON으로 변환
        }
        try:
            response = requests.post(server_url, json=data)
            if response.status_code == 200:
                print(f"Parameters successfully sent from {client_id}.")
            else:
                print(f"Failed to send parameters from {client_id}. Status Code: {response.status_code}")
        except requests.exceptions.RequestException as e:
            print(f"An error occurred while sending parameters: {e}")
